...
Here, I display the first fifteen lines of the `admissions.csv.gz` file:

```{bash}
# Display the first 5 lines in the file
zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz | head -n 15
```
...
```{bash}
# Count the rows in the data file (following the example above)
...
Now, let's identify the possible values for each of the listed variables:

```{bash}
# Looping over column numbers
# corresponding to the variables of interest

# First, create a data variable to keep code short
data='/usr/203b-data/mimic-iv/core/admissions.csv.gz'

# No loop version would be this:
# zcat $data | head -n 1 | awk -F, '{print $6}'
# zcat $data | tail -n +2 | awk -F, '{print $6}' | sort | uniq -c
# awk need to distinguish between the internal $ and the loop $
# this is the option -v below

# Then loop over variables
# the awk command requires the -v
for i in 6 7 9 10 11 12
  do
    echo "Unique values and counts in variable in"
    zcat $data | head -n 1 | awk -v i="$i" -F, '{print $i}'
    zcat $data | tail -n +2 | awk -v i="$i" -F, '{print $i}' | sort | uniq -c
  done
```
