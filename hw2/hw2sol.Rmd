...
dchartitems <- 
  ...
  as_tibble()
# class(dchartitems$itemid) # Integer
print(dchartitems, width = Inf)
```

Now, let's select the items of interest:

```{r, eval=FALSE}
labitems <- "Creatinine"
creatinine_items <- tibble %>%
  filter(str_detect(label, regex(labitems, ignore_case = TRUE))) %>%
  print()
```
...
```{r, eval=FALSE}
# First, let's load and filter the admissions data
icu_first <- 
  fread(paste0(mimic_path,"/icu/icustays.csv.gz"))
  admissions %>%
  # Format as date, then numeric
